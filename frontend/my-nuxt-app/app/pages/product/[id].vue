<template>
  <div class="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-16">
    <div class="product-detail max-w-5xl mx-auto p-8 bg-gradient-to-b from-slate-800 via-slate-700 to-slate-800 text-white rounded-2xl shadow-xl animate-fadeIn">
      <div v-if="loading" class="text-center text-gray-300 text-xl py-12">Loading...</div>

      <div v-else-if="product" class="product-container flex flex-wrap gap-8">
        <!-- Image Section -->
        <div class="image-section flex-1 min-w-[300px] text-center">
          <img
            :src="product.image"
            :alt="product.name"
            class="product-image w-full max-h-[350px] object-contain rounded-xl transition-transform duration-300 hover:scale-105"
          />
        </div>
        ...
        <div class="info-section flex-1 min-w-[400px] text-left">
          ...
          <div v-if="product.discount_price && product.discount_price > 0" class="discount-wrapper mb-4">
            <span
              class="discount-badge inline-block bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-bold mb-1 animate-flash"
            >
              🔥 SALE – You save {{ ((product.discount_price * 100 / product.price )).toFixed(0) }}%
            </span>
            <p class="product-price text-blue-400 text-lg mt-1">
          ...
          <p
            :class="[
              'stock-info font-medium mt-2',
              product.stock === 0 ? 'text-red-600 font-bold' : product.stock < 5 ? 'text-orange-400 font-bold' : ''
            ]"
          >
            <strong>Stock:</strong>
