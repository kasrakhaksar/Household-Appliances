<template>
  <div class="container mx-auto px-6 py-16 relative z-10">
    ...
    <div v-else class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      ...
      >
        <div
          class="group relative bg-slate-900 border border-slate-700
        ...
        >
          ...
          <!-- Product Info -->
          <h5 class="font-bold text-white text-lg mb-2 truncate">{{ product.name }}</h5>
    ...
    <div class="absolute top-[-10%] left-[-10%] w-96 h-96 bg-blue-500/20 rounded-full blur-3xl animate-float-slow"></div>
    <div class="absolute bottom-[-10%] right-[-10%] w-72 h-72 bg-pink-400/20 rounded-full blur-2xl animate-float-slow"></div>
  </div>
</template>
...
onMounted(async () => {
  try {
    products.value = await ProductAPI.getHomeProducts(8)
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
